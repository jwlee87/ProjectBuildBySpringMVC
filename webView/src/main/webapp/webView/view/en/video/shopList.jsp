...
and (min-device-width : 570px) {
	...
	#topcon {height: 25%;}
	#shopcon {height: 70%; margin: 8px 0 0 1%; padding: 0 1%;} 
	...
	.thumnail {width: 88%; margin: 5%;}
	#navirow {height: auto; padding-top: 12px;}
	#leftBtn, #rightBtn {height: 25px; margin: 0 15px;}
	#currentPage {font-size: 20px; text-decoration: none; font-weight: bold; text-align: center; margin-left: 44px;}
	#currentPage a {color: gold;}
	nav {width: 100%; text-align: center; height: 100%;}
}
	...
	<div class="contentcon" style="height: 100%;">
		...
		<div class="col-12">
				...
				<div class="row" style="height: 100%;">
					<div class="col-6">
						<a href="/web/video/shop?id=${member.id}&cp=1">
							<img id="shopIcon" src="/webView/img/common/videoList/shop_icon_on.png"/>
						</a>
					</div>
					<div class="col-6">
						<a href="/web/video/my?id=${member.id}&cp=1">
							<img id="myIcon" src="/webView/img/common/videoList/my_icon_off.png"/>
						</a>
					</div>
				</div>
			</div>
			...
			<div id="navirow" class="row">
				<div class="col-4"></div>
				<div id="currentPage" class="col-4">
		...
		<!-- video page -->
		<div id="shopcon" class="row">
			<div class="col-12" style="padding:0 2% 2% 2%; overflow: scroll; height: 100%;">
				
				
				<c:forEach var="video" items="${list}">
				<a href="/web/video/videoDetail?id=${member.id}&no=${video.videoNo}">
					<div class="row" style="height: 53%; width: 99%; margin: 0 auto 1% auto; background-color: #3E2908;">
						<div class="col-3" style=" padding: 0 2% 0 2%;">
							<c:if test="${video.price eq 6}">
								<img id="freeicon" src="/webView/img/common/shopList/coin_6.png"/>
							</c:if>
							<c:if test="${video.price eq 8}">
								<img id="freeicon" src="/webView/img/common/shopList/coin_8.png"/>
							</c:if>
							<c:if test="${video.price eq 10}">
								<img id="freeicon" src="/webView/img/common/shopList/coin_10.png"/>
							</c:if>
							<c:if test="${video.price eq 12}">
								<img id="freeicon" src="/webView/img/common/shopList/coin_12.png"/>
							</c:if>
							<c:if test="${video.price eq 14}">
								<img id="freeicon" src="/webView/img/common/shopList/coin_14.png"/>
							</c:if>
							<c:if test="${video.price eq 16}">
								<img id="freeicon" src="/webView/img/common/shopList/coin_16.png"/>
							</c:if>
						</div>
...
function goPage(pageNo,id) {
	location.href = "/web/video/shop?id="+id+"&cp="+pageNo;
}
