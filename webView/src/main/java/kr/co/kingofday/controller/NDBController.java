...
public class NDBController {
	...
	public void responseReward(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		reward.setUserUniqueID(Integer.parseInt(usn));
		reward.setReward_key(Integer.parseInt(reward_key));
		...
		if( signed_value.equals(check_value)) 
		{
			...
			ArrayList<Reward> list = rewardService.getRewardList(reward);
			...
			if(list.size() != 0)
			{
				for(Reward rwd: list) {
					...
					if( rwd.getReward_key() == Integer.parseInt(reward_key) ) {
		...
		{
			logger.info("signed_value 체크 실패");
